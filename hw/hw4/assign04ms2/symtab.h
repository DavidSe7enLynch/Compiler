...
public:
    // for vreg
    Storage(StorageKind kind, int vreg);
    // for memory
    Storage(StorageKind kind, unsigned memOffset);

    Storage(StorageKind kind, std::string globalName);

//    void setStorage(StorageKind kind, int vreg);
//
//    void setStorage(StorageKind kind, unsigned memOffset);
...
public:
    Symbol(SymbolKind kind, const std::string &name, const std::shared_ptr <Type> &type, SymbolTable *symtab,
           bool is_defined);
    ...
    void setStorage(StorageKind kind, std::string globalName);

    const Storage &getStorage() const;

    bool isGlobal() const;

    void setReqStorKind(StorageKind kind);

    StorageKind getReqStorKind();
};
